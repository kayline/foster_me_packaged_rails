class FosterFamiliesController < ApiController
	...
	def create
		...
		@animals = animals_params.map { |params| Animal.new(params) }
	...
	def animals_params
		animals = params.require(:family).permit(animals: [:name, :sex, :description, :date_of_birth, :profile_photo])['animals']
		if animals.present?
			animals
		else
			[]
